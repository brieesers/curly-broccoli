...
namespace LuaSTGEditorSharp.EditorData
{
    ...
    public abstract class TreeNode : INotifyPropertyChanged, ICloneable, IMessageThrowable
    {
        ...
        protected TreeNode()
        {
            Logger = EditorLogging.ForContext($"{this.GetType().Name} (TreeNode)");
        ...
        {
            bool isValidated = ValidateChild(toV, this);
            if (isValidated)
                Logger.Verbose($"Node {toV} was validated.");
            else
                Logger.Verbose($"Node {toV} wasn't validated.");
        ...
        private bool ValidateChild(TreeNode toV, TreeNode originalParent)
        {
            if (this is Folder 
                ...
                || this is FolderGreen 
                || this is FolderBlue 
                || this is FolderYellow)
                return GetLogicalParent()?.ValidateChild(toV, originalParent) ?? true;
            if (toV is Folder || toV is FolderRed || toV is FolderGreen || toV is FolderBlue || toV is FolderYellow)
            {
                ...
                {
                    if (!ValidateChild(t, originalParent))
                        return false;
                }
                return true;
            }
            if (isCustomNode && isCustomNodeLeaf)
                return false;
            else if (PluginHandler.Plugin.NodeTypeCache.NodeTypeInfo[GetType()].leaf)
                return false;
            ...
            {
                cur = stack.Pop();
                if (!cur.FindAncestorIn(cur, toV.Parent, this, null))
                    return false;
                if (PluginHandler.Plugin.NodeTypeCache.NodeTypeInfo[cur.GetType()].classNode)
                {
                    if (!(MatchClassNode(cur, toV.Parent) || MatchClassNode(this, null)))
                        return false;
                }
        ...
        {
            Children.Add(n);
            Logger.Verbose($"Node {n} added as Children of {this}.");
            //n._parent = this;
        ...
        public void InsertChild(TreeNode n, int index)
        {
            ...
            Logger.Verbose($"Node {n} added as Children of {this} at index {index}.");
            //n._parent = this;
            //n.RaiseCreate(new OnCreateEventArgs() { parent = this });
        }
        ...
        {
            Children.Remove(t);
            Logger.Verbose($"Node {t} removed as children of {this}.");
            //t.RaiseRemove(new OnRemoveEventArgs() { parent = this });
        }
